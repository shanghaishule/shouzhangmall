...
class IndexController extends Controller {
    ...
    public function pro($id){
    	if(is_numeric($id)) {
	    	if(mc_user_id()==$id) {
		    	$this->page = M('action')->where("user_id='".mc_user_id()."' AND action_key IN ('trade_wait_send','trade_wait_cofirm','trade_wait_finished','trade_wait_hdfk')")->order('id desc')->select();
